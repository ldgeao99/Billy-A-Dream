...
public class SendEMAILController {
	...
	public String email(@RequestParam("email") String email, HttpSession session) throws Exception {
		...
		int EmailAuthNum = sendSMS.randomRange(10000, 99999);// 갯수
		String subject = "[Biily A Dream] 인증번호 발송";
		String msg = "[Billy A Dream] 인증번호 : [" + EmailAuthNum + "] 입니다.";

		sendEMAIL.sendMail(email, subject, msg);

		System.out.println("부여한 인증번호 :" + EmailAuthNum);
	...
	public String authNumCheck(@RequestParam("EmailauthNum") int EmailauthNum, HttpSession session) {
		...
		System.out.println("입력한 입증번호 :" + getEmailAuthNum);
		System.out.println("부여한 인증번호 :" + EmailauthNum);
